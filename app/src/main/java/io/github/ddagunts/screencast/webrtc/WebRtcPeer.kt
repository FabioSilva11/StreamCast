...
class WebRtcPeer(
    ...
    fun build() {
        logI("webrtc peer build: audio=${config.audioEnabled} preset=${config.videoPreset.label} bitrate=${config.maxBitrateBps}")
    ...
    fun addScreenSource(
        ...
        logI("webrtc screen capture started: ${preset.width}x${preset.height}@${preset.fps}")
    ...
    private val observer = object : PeerConnection.Observer {
        ...
        override fun onIceCandidate(candidate: IceCandidate) {
            logI("local ICE candidate: mid=${candidate.sdpMid} index=${candidate.sdpMLineIndex}")
