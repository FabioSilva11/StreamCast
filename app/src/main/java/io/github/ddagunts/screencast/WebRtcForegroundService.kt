...
class WebRtcForegroundService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        createChannel()
        createRemoteVolumeSession()
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        logI("webrtc service command action=${intent?.action} startId=$startId")
        ...
        sessionJob = scope.launch {
            ...
            launch {
                s.state.collect { st ->
                    ...
                    refreshNotification()
                    if (st !is WebRtcState.Idle) {
                        sessionBecameActive = true
                    }
                    if (sessionBecameActive && st is WebRtcState.Idle && session === s) {
                        logW("webrtc: session became idle; clearing stale session for retry")
                        session = null
                        releaseWakeLock()
                        stopSelfIfIdle()
                    } else if (st is WebRtcState.Error) {
            ...
            launch {
                s.volume.collect { v ->
                    volumeProvider?.setCurrentVolume((v.level * 100).toInt().coerceIn(0, 100))
    ...
    private fun stopCast() {
        logW("webrtc service stopCast invoked")
        sessionJob?.cancel()
        sessionJob = null
    ...
    override fun onDestroy() {
        logW("webrtc foreground service onDestroy")
        stopCast()
        scope.cancel()
    ...
    private fun createRemoteVolumeSession() {
        val provider = object : VolumeProvider(VOLUME_CONTROL_ABSOLUTE, 100, 100) {
            override fun onAdjustVolume(direction: Int) {
                val current = _session.value?.volume?.level ?: return
                val next = when (direction) { -1 -> current - 0.05; 1 -> current + 0.05; else -> current }.coerceIn(0.0, 1.0)
                logI("hardware volume background WebRTC adjust: direction=$direction next=$next")
                scope.launch { runCatching { session?.setVolume(next) } }
            }
            override fun onSetVolumeTo(volume: Int) {
                val next = volume.coerceIn(0, 100) / 100.0
                logI("hardware volume background WebRTC set: next=$next")
                scope.launch { runCatching { session?.setVolume(next) } }
            }
        }
        volumeProvider = provider
        mediaSession = MediaSession(this, "StreamCast WebRTC").apply {
            setPlaybackToRemote(provider)
            isActive = true
        }
        logI("remote volume MediaSession active")
    }
