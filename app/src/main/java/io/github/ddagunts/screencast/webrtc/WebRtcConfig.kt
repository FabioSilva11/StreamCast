...
enum class VideoPreset(val label: String, val width: Int, val height: Int, val fps: Int) {
    ...
    companion object {
        // 1080p60 can complete WebRTC negotiation but fail to render on
        // Chromecast-compatible TCL receivers. 1080p30 is the Cast-safe
        // baseline documented for broad receiver compatibility.
        val DEFAULT = FHD_30
...
const val DEFAULT_WEBRTC_BITRATE_MBPS = 8
...
class WebRtcConfigStore(context: Context) {
    ...
    fun snapshot(): WebRtcSessionConfig = WebRtcSessionConfig(
        videoPreset = videoPreset.takeUnless { it == VideoPreset.FHD_60 } ?: VideoPreset.FHD_30,
        maxBitrateBps = maxBitrateMbps.coerceAtMost(8) * 1_000_000,
